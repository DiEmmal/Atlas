...
export class UserController {
    ...
    public getUserImage = (req: Request, res: Response) => {
        ...
        return getUserImageUseCase.execute(userID)
    ...
    public updateUser = async (req: Request, res: Response) => {
        ...
        if(userID !== req.body.user.id) return res.status(403).json({ error: 'You are not authorized to update this user' });
        ...
        return updateUserUseCase.execute(dto!, userID!, image)
            .then(user => res.status(200).json({
                ...
                user: { ...user, imgURL: user.img ? `${envs.WEBSERVICE_URL}/users/${user.id}/image` : undefined },
            }))
            .catch(error => this.errorService.HandleHttpError(error, res));
